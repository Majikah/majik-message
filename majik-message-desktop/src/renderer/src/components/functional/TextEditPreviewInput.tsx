...
const TextEditPreviewInput: React.FC<TextEditPreviewInputProps> = ({
  ...
  const selectFile = (accept: string): Promise<File> =>
    new Promise((resolve, reject) => {
      ...
      el.accept = accept
      el.onchange = () =>
        el.files?.[0] ? resolve(el.files[0]) : reject(new Error('No file selected'))
      el.click()
    })
  ...
  const handleDownloadJson = (): void => {
    try {
      const blob = new Blob([JSON.stringify({ original: input, encrypted: output })], {
  ...
  const labels = useMemo(
    () => ({
      ...
      outputPlaceholder: isProcessing ? 'Processing…' : 'Output will appear here…',
      ...
      <EditorGrid>
        ...
        <EditorPane $accent={null} id="panel-output-pane">
          <PaneHeader>
            ...
            <PaneStatusBadge $mode={output ? mode : null}>{labels.outputBadge}</PaneStatusBadge>
          </PaneHeader>
