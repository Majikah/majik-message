...
const TrackFill = styled.div<{ $pct: number; $critical: boolean; $warn: boolean }>`
  height: 100%;
  border-radius: 99px;
...
const UserFileQuota: React.FC<UserFileQuotaProps> = ({ quota, isLoading = false, className }) => {
  ...
  const stats = [
    { dot: accentColor, val: `${quota?.usedMB.toFixed(1) ?? '–'} MB`, label: 'Used' },
    { dot: '#f5a623', val: `${rawPct.toFixed(1)}%`, label: 'of Quota' },
    { dot: '#3ecf8e', val: `${quota?.availableMB.toFixed(1) ?? '–'} MB`, label: 'Available' }
  ]
  ...
  return (
    <Card className={className}>
      ...
      <TrackWrap>
        <TrackFill $pct={isLoading ? 0 : fillPct} $critical={isCritical} $warn={isWarn} />
      </TrackWrap>
