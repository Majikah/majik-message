...
const CBaseChatBubble: React.FC<CBaseChatBubbleProps> = ({
  ...
  // ── Derived values ─────────────────────────────────────────────────────────
  const time = useMemo(() => moment(new Date(message.getTimestamp())).fromNow(), [message])

  const fullTime = useMemo(
    () => moment(new Date(message.getTimestamp())).format('MMM D, YYYY [at] h:mm A'),
    [message]
  )
  ...
  const handleShare = async (): Promise<void> => {
    ...
    try {
      ...
      // Native Notification
      window.electron.notify('Copied to clipboard', raw)
    } catch (err) {
      toast.error('Failed to copy', {
  ...
  const hasActions =
    canShare || canDownload || (!!onDelete && canDelete && isOwn) || (!!onEdit && canEdit && isOwn)
        ...
        <BubbleRow $isOwn={isOwn}>
          <Actions $enabled={hasActions}>
            ...
            {!!onDelete && canDelete && isOwn && (
              <DeleteButton title="message" onClick={() => onDelete?.(message)} />
            )}
          </Actions>
