...
const Card = styled.div<{ $isActive: boolean }>`
  ...
  &::before {
    ...
    right: 0;
    height: 4px;
    background: ${({ $isActive, theme }) => ($isActive ? theme.gradients.strong : 'transparent')};
...
const AccountName = styled.h3<{ $blocked: boolean }>`
  ...
  color: ${({ $blocked, theme }) => ($blocked ? theme.colors.error : theme.colors.textPrimary)};
  margin: 0;
  white-space: nowrap;
  overflow: hidden;
...
const KeyBox = styled.div<{ $isActive: boolean }>`
  ...
  border: 1px solid
    ${({ $isActive }) => ($isActive ? 'rgba(79, 110, 247, 0.18)' : 'rgba(255, 255, 255, 0.05)')};
  border-radius: 7px;
  padding: 8px 10px;
  opacity: 0.75;
  transition:
