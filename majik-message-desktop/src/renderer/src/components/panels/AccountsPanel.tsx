...
const AccountsPanel: React.FC<AccountsPanelProps> = ({ majik, onUpdate }) => {
  ...
  const handleLoadMnemonicAccount = async (): Promise<void> => {
    ...
    try {
      ...
      window.electron.notify(
        ...
        `New Account for ${label || mnemonicJSON.id} created successfully.`
      )
      onUpdate?.(majik)
      setRefreshKey((k) => k + 1)
    } catch (e) {
  ...
  const processRegisterOnline = async (contact: MajikContact): Promise<string> => {
    if (contact.isMajikahRegistered()) throw new Error('Already registered online.')
      ...
      <Body>
        {userAccounts.length > 0 ? (
          <Grid>
            {userAccounts.map((account, index) => (
              <CBaseUserAccount
                ...
                onSetActive={(item) =>
                  majik?.isAccountActive(item.id) ? undefined : handleSetAsActive(account)
                }
          ...
          <EmptyState>
            ...
            <EmptyHint>Create or import an account to start using Majik Message.</EmptyHint>
          </EmptyState>
        )}
      </Body>
    </Root>
