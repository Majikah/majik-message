...
const FilePanel: React.FC<FilePanelProps> = ({ majik }) => {
  ...
  useEffect(() => {
    const state = location?.state as {
      pendingFile?: { bytes: number[]; name: string };
      mode?: EditorMode;
    } | null;

    if (!state?.pendingFile) return;

    const { bytes, name } = state.pendingFile;
    ...
    // Inject the file into FileVault
  ...
  const handleDecryptFile = async (file: File): Promise<DecryptResult> => {
    ...
    const { bytes, originalName, mimeType } = await majik.decryptFile({
      source: file,
    });
      ...
      try {
        ...
        if (signerKey?.hasSigningKeys) {
          ...
          const contact = allContacts.find(
            (c) => c.fingerprint === result.signerId,
          );
    ...
    return {
      binary: new Blob([bytes as BlobPart], {
    ...
    <PageRoot>
      ...
      <PanelHeader>
        <HeaderLeft>
          ...
          <PanelSubtitle>
            Local Encrypt &amp; Decrypt · Works Offline · .mjkb format
          </PanelSubtitle>
          <HeaderBadge>
            <LiveDot />
      ...
      <DescChip>
        ...
        <span>
          Encrypt any file into a sealed <strong>.mjkb</strong> binary —
          protected with <strong>ML-KEM-768 + AES-256-GCM</strong>. Works fully
          offline. Drop in a <strong>.mjkb</strong> to decrypt it back to the
          original file. No size limit for local use.
        </span>
      </DescChip>
      ...
      {!myAccount ? (
        ...
        <Body>
          ...
          <FileVault
            ...
            decryptFile={mode === "decrypt" ? importedFile : undefined}
            encryptFile={mode === "encrypt" ? importedFile : undefined} // ← add this
