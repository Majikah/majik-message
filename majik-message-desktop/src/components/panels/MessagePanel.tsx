  ...
  const handleDecryptMessage = async (input: string): Promise<string> => {
    ...
    try {
      ...
      const fingerprintMap = new Map(
        allContacts.map((c) => [c.fingerprint, c]),
      );
    ...
    <PageRoot>
      ...
      <PanelHeader>
        <HeaderLeft>
          ...
          <PanelSubtitle>
            Local Encrypt &amp; Decrypt · Works Offline
          </PanelSubtitle>
        </HeaderLeft>
      ...
      <DescChip>
        ...
        <span>
          Encrypt messages for any contact — even without Majikah registration.
          Works fully offline. For live conversations, use the{" "}
          <strong>Chats</strong> window.
        </span>
      </DescChip>
      ...
      {!myAccount ? (
        <EmptyWrap>
          <DynamicPlaceholder>
            Please create an account first to start encrypting and decrypting
            messages.
          </DynamicPlaceholder>
