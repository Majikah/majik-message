...
const RootContainer = styled.li<{ $isUnread: boolean }>`
  ...
  &::before {
    ...
    background: ${({ $isUnread, theme }) =>
      $isUnread ? theme.colors.primary : "transparent"};
...
const StarBtn = styled.button<{ $isStarred: boolean }>`
  ...
  flex-shrink: 0;
  color: ${({ $isStarred }) =>
    $isStarred ? "#f59e0b" : "rgba(255, 255, 255, 0.18)"};
  transition: color 150ms ease;

  &:hover {
    color: ${({ $isStarred }) =>
      $isStarred ? "#f59e0b" : "rgba(255,255,255,0.5)"};
  }
`;
...
const ThreadRow: React.FC<ThreadRowProps> = ({
  ...
  const subject =
    thread.subject ||
    thread?.latest_message?.metadata?.subject ||
    "(No Subject)";
  ...
  return (
    ...
    >
      ...
      <Actions className="tr-actions">
        {/* Slots ready for delete / mark-read buttons */}
      </Actions>
    </RootContainer>
  );
};
