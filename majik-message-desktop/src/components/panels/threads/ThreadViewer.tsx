...
import { launchTutorialThreadsMessages } from "@/lib/shepherd-js/tutorials/tutorial-threads";
...
export const ThreadViewer: React.FC<ThreadViewerProps> = ({
  ...
  const [threadLabel, setThreadLabel] = useState<string | undefined>(
      ...
      const entries = await Promise.all(
        participantKeys.map(async (key) => {
          try {
            ...
            return [
              key,
              contact?.meta?.label || key.slice(0, 6) + "…" + key.slice(-4),
            ] as const;
          } catch {
  ...
  const statusVariant = ():
    ...
    if (thread.status === ThreadStatus.PENDING_DELETION)
        ...
        <TitleRow>
          <TitleLeft>
            ...
            <ParticipantChips>
              ...
              {participantChips.rest > 0 && (
                <KeyChip>+{participantChips.rest} more</KeyChip>
              )}
            </ParticipantChips>
          </TitleLeft>
      ...
      <ScrollArea ref={scrollAreaRef} id="section-thread-messages">
        {fetchedMails.length === 0 ? (
          <EmptyState>
            ...
            <EmptyMessage>
              This thread has no messages. Send the first one above.
            </EmptyMessage>
          </EmptyState>
        ) : (
          <>
            {fetchedMails.length > 1 && (
              <MailCountLabel>
                {fetchedMails.length}{" "}
                {fetchedMails.length === 1 ? "message" : "messages"}
              </MailCountLabel>
            )}
