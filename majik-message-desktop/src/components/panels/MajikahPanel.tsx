...
const MajikahPanel: React.FC<MajikahPanelProps> = ({ majik }) => {
  ...
  const [currentIdentities, setCurrentIdentities] = useState<
    MajikMessageIdentity[]
  >([]);
  const [selectedAccount, setSelectedAccount] = useState<MajikContact | null>(
    null,
  );
  ...
  // ── Delete identity ────────────────────────────────────────────────────────
  const processDeleteIdentity = async (
  ...
  const processDeleteUserData = async (): Promise<string> => {
    if (!majikah.user) {
      throw new Error(
        "There seems to be a problem with the authenticated user.",
      );
    }
  ...
  return (
    <PageRoot>
      ...
      <Section>
        ...
        <WindowDataTable
          key={refreshKey}
          columns={columnsAccountIdentities(
            undefined,
            undefined,
      ...
      <Section>
        <DangerCard>
          ...
          <DangerBody>
            <DangerDescription>
              Permanently deletes your Majikah account, all registered
              identities, and associated user data. This action cannot be
              undone.
            </DangerDescription>
