...
const PRESET_META: Record<
  ...
  { label: string; hint: string; level: number }
> = {
  FASTEST: {
    label: "Fastest",
...
const FileRow = styled.div<{ $invalid?: boolean }>`
  ...
  border: 1px solid
    ${({ $invalid }) =>
      $invalid ? "rgba(239, 68, 68, 0.3)" : "var(--border, #1c1c22)"};
  border-radius: 8px;
  transition: border-color 200ms ease;
`;
...
const OutputArea = styled.div<{ $hasResult: boolean }>`
  ...
  justify-content: ${({ $hasResult }) =>
    $hasResult ? "flex-start" : "center"};
...
const ScanResultRow = styled(ResultRow)<{ $band?: FileScanResult["riskBand"] }>`
  border-color: ${({ $band }) =>
    $band ? `${RISK_BAND_COLOR[$band]}33` : undefined};
  background: ${({ $band }) => ($band ? RISK_BAND_BG[$band] : undefined)};
`;

function scanResultAccent(
  phase: ScanPhase,
): "clean" | "flagged" | "warn" | "neutral" {
...
const FileVault: React.FC<FileVaultProps> = ({
  ...
  const [encryptSignerInfo, setEncryptSignerInfo] = useState<
    SignerInfo | null | undefined
  >(undefined);
  const [decryptSigStatus, setDecryptSigStatus] =
    useState<DecryptSignatureStatus | null>(null);
  ...
  const [selectedPreset, setSelectedPreset] =
    useState<CompressionPresetKey>("GOOD");
  ...
  useEffect(() => {
    ...
    listen<{ paths: string[] }>("tauri://drag-drop", async (event) => {
      ...
      try {
        ...
        // Read bytes directly — no fetch, no asset protocol, no CSP issue
        ...
        toast.error("Failed to load dropped file", {
  ...
  const validateEncryptFile = (
    file: File,
  ): { valid: boolean; mime: string; msg?: string } => {
  ...
  const acceptEncryptFile = async (file: File): Promise<void> => {
    ...
    const looksLikeMjkb =
      file.name.toLowerCase().endsWith(".mjkb") ||
      (await isFullyValidMjkbFile(file));
    ...
    if (looksLikeMjkb) {
      ...
      toast.info("Switched to Decrypt mode", {
        description:
          "This file is an encrypted .mjkb — loading it for decryption.",
  ...
  const acceptDecryptFile = async (file: File): Promise<void> => {
    ...
    if (!looksLikeMjkb) {
      ...
      toast.info("Switched to Encrypt mode", {
        description:
          "This file is not an encrypted .mjkb — loading it for encryption.",
    ...
    if (mode === "encrypt" && encryptResult) {
      ...
      if (scanPhase === "flagged") {
        toast.warning("Downloading flagged file — proceed with caution", {
          description:
            "A YARA rule matched this file. Save to an isolated location.",
  ...
  const compressionRatio =
    encryptResult && encryptResult.originalSize > 0
      ? Math.round(
          (1 - encryptResult.encryptedSize / encryptResult.originalSize) * 100,
        )
      : null;
  ...
  const downloadVariant =
    scanPhase === "flagged" ? "warn" : hasResult ? mode : undefined;
      ...
      <EditorGrid id="file-vault-editor-grid">
        ...
        <EditorPane $accent={inputAccent} id="file-vault-input-pane">
          <PaneHeader>
            ...
            <PaneStatusBadge
              $variant={!inputFile ? null : inputFile.valid ? mode : "error"}
            >
            ...
            {!inputFile ? (
              ...
              <FileCard onClick={(e) => e.stopPropagation()}>
                <FileRow $invalid={!inputFile.valid}>
                  ...
                  <FileClearBtn onClick={handleClearInput}>
                    ✕ Remove
                  </FileClearBtn>
                </FileRow>
                {inputFile.valid ? (
                  <MimeRow>
                    ...
                    <MimeChip
                      $variant={mode === "encrypt" ? "source" : "mjkb"}
                      data-private
                    >
                      {inputFile.mime}
                    </MimeChip>
          ...
          {mode === "encrypt" && inputFile?.valid && (
            <CompressionWrap id="file-vault-compression-selector">
              <CompressionHeaderRow>
                ...
                <CompressionHint>
                  {PRESET_META[selectedPreset].hint}
                </CompressionHint>
              </CompressionHeaderRow>
              ...
              {adaptiveInfo?.wasClamped && (
                <ClampNotice>
                  ...
                  <span>
                    {selectedPreset} (lv {CompressionPreset[selectedPreset]})
                    will be auto-clamped to lv {adaptiveInfo.effective} for this
                    file size to prevent out-of-memory errors.
                  </span>
                </ClampNotice>
              )}
            </CompressionWrap>
        ...
        <EditorPane $accent={outputAccent} id="file-vault-output-pane">
          ...
          {!isProcessing && decryptedIsNestedMjkb && (
            <NestedMjkbBanner>
              ...
              <span>
                <strong>Nested .mjkb detected.</strong> The decrypted file is
                itself an encrypted .mjkb — this file was double-encrypted.
                Download it and decrypt again to recover the original plaintext.
                By design, Majik Signature does not permit encrypting an already
                encrypted .mjkb.
              </span>
            </NestedMjkbBanner>
          )}
          ...
          <OutputArea $hasResult={isProcessing || hasResult}>
                ...
                <ProcessSteps>
                  {steps.map((step) => (
                    <MimeChip
                      key={step.label}
                      $variant={
                        step.done ? "valid" : step.active ? "source" : "neutral"
                      }
                    >
            ...
            {!isProcessing &&
              ...
              (() => {
                ...
                const desiredLevel = CompressionPreset[
                  selectedPreset
                ] as number;
              ...
              <ResultCard>
                ...
                <ResultRow>
                  ...
                  <ResultValue
                    $accent={
                      !decryptSigStatus ||
                      decryptSigStatus.verdict === "unsigned"
                        ? "neutral"
                ...
                {decryptedIsNestedMjkb && (
                  <ResultRow>
                    ...
                    <ResultValue $accent="warn">
                      ⚠ Nested .mjkb — decrypt again
                    </ResultValue>
                  </ResultRow>
                )}
                {scanPhase !== "idle" && scanResult && (
        ...
        <StatsRow id="file-vault-stats-row">
          ...
          <StatChip>
            ...
            <StatValue $accent={mode}>
              {mode === "encrypt" ? "ML-KEM-768" : "✓ Verified"}
            </StatValue>
          </StatChip>
        </StatsRow>
      )}
