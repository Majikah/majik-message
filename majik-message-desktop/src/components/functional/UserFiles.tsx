...
import type {
  FileQuota,
  UploadIntentBody,
} from "../majikah-session-wrapper/types/files-api";
...
const DropZone = styled.div<{ $active: boolean }>`
  border: 1.5px dashed
    ${({ $active, theme }) =>
      $active ? theme.colors.primary : theme.colors.primaryBackground};
  border-radius: 12px;
  padding: 20px;
  text-align: center;
  background: ${({ $active, theme }) =>
...
const ConfirmBtn = styled.button<{ $ready: boolean }>`
  ...
  font-weight: 500;
  border: 1px solid
    ${({ $ready }) => ($ready ? "rgba(62,207,142,0.25)" : "transparent")};
...
const DurationChip = styled.button<{ $active: boolean }>`
  ...
  flex-shrink: 0;
  background: ${({ $active }) =>
    $active ? "rgba(245,166,35,0.15)" : "transparent"};
  color: ${({ $active, theme }) =>
    $active ? "#f5a623" : theme.colors.textSecondary};
  border-color: ${({ $active }) =>
...
const UserFiles: React.FC<UserFilesProps> = ({
  ...
  const activeContext: FileContext =
    tab === "attachments" ? "thread_attachment" : uploadContext;
  ...
  const signerInfoMap = useMemo(() => {
    const map = new Map<
      string,
      ReturnType<typeof majik.getMajikFileSignerInfo>
    >();
    for (const file of files) {
      ...
      try {
        map.set(
          file.id,
          majik.getMajikFileSignerInfo(MajikFile.fromJSON(file)),
        );
      } catch {
        map.set(file.id, null);
      }
  ...
  const encryptPendingFileById = useCallback(
    async (pf: PendingFile) => {
      ...
      try {
        ...
        setPendingFiles((prev) =>
          prev.map((p) => (p.id === pf.id ? { ...p, majikFile } : p)),
        );
      } catch (e) {
  ...
  const processTogglePublish = async (
    ...
    setFiles((prev) =>
      prev.map((f) =>
        f.id === fileId
          ? { ...f, is_shared: result.isShared, share_token: result.shareToken }
          : f,
      ),
    );
    if (result.isShared) {
      return {
        ...
        description:
          "Anyone with the link can now access this file on the web.",
      };
    }
    return {
  ...
  const copyShareLink = async (file: MajikFileJSON): Promise<void> => {
    if (!file.is_shared || !file.share_token) {
      toast.info("File is not published", {
        description:
          "Use the publish button (globe icon) to make it web-accessible first.",
  ...
  const processDownloadFile = async (file: MajikFileJSON): Promise<string> => {
    ...
    const majikFileInstance = await MajikFile.fromJSONWithBlob(
      file,
      downloadedBlob,
    );
    ...
    a.href = url;
    let fileName =
      majikFileInstance.originalName ?? file.original_name ?? "download";
    ...
    <Root id="my-files-root">
      ...
      <Controls>
        <TabBar id="my-files-tab-bar">
          {(
            ["all", "permanent", "temporary", "shared", "attachments"] as Tab[]
          ).map((t) => (
      ...
      <ScrollBody>
        ...
        >
          ...
          <DropHint>
            YARA-scanned · encrypted with MajikFile · max 100 MB · .mjkb files
            not accepted
          </DropHint>
        </DropZone>
        ...
        {pendingFiles.length > 0 && (
          <div id="my-files-pending-section">
            ...
            {pendingFiles.map((pf) => {
              ...
              const adaptiveInfo = resolveAdaptiveLevel(
                pf.raw.size,
                pf.compressionPreset,
              );
                ...
                <PendingRow key={pf.id} $scanBlocked={scanBlocked}>
                  <RowColumn>
                    <RowInfo>
                      ...
                      <DurationRow
                        data-visible={
                          scanPassed && pf.storageType === "temporary"
                            ? "true"
                            : "false"
                        }
                      >
                        <DurationLabel>ttl</DurationLabel>
                        {([1, 2, 3, 5, 7, 15] as TempFileDuration[]).map(
                          (d) => (
                            <DurationChip
                              key={d}
                      ...
                      <CompressionWrap>
                        ...
                        {adaptiveInfo.wasClamped && (
                          <ClampNotice>
                            ...
                            <span>
                              {pf.compressionPreset} (lv{" "}
                              {CompressionPreset[pf.compressionPreset]}) will be
                              auto-clamped to lv {adaptiveInfo.effective} for
                              this file size.
                            </span>
                          </ClampNotice>
                        )}
        ...
        <div id="my-files-file-list">
          {loading ? (
            ...
            <>
              <SectionLabelRow
                style={{ marginTop: pendingFiles.length > 0 ? 16 : 4 }}
              >
                <SectionLabelText>
